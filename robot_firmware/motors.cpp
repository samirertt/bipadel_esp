...
void motors_enable() {
  for (int id = 1; id <= 6; id++) {
    ...
    if (id >= 3) {
      // Legs: Position Mode (3), Direct Passthrough (1).
      // Gains and limits for nodes 5/6 are configured externally via
      // the Motor Wizard and persist in ODrive flash.  We do not
      // push them over CAN at boot.
      mode_data[0] = 3;
      mode_data[4] = 5;
    }
...
void motors_set_leg_positions_deg(float l_knee_deg, float r_knee_deg,
                                  float l_torso_deg, float r_torso_deg) {
  ...
  // Pure position commands.  Vel_FF and Torque_FF default to 0 per
  // the can_bus_send_position signature in can_bus.h.
